...
def surf_info_fetch():
    ...
    for url in surf_url_list:
        ...
        # Wave Height
        try:
            # TEST CASE # 2 when wave height from URL is listed as flat
            # We want to ensure that the message doesn't read as 'flat ft'
            wave_height = 'flat' + " ft"
        except Exception:
...
if __name__ == '__main__':
    ...
    if len(surf_forecasts) > 0:
        ...
        for i in surf_forecasts:
            publish_message(kafka_producer, 'surf_data_topic555', 'raw', i.strip())
